...
// Función para convertir strings → números y null → 0
...
router.get('/:id', authenticateToken, async (req, res) => {
  try {
    ...
    res.json(normalizarNumeros(rows[0]));
...
// ===============================================
// GET /vehiculos/:id/estadisticas — Stats del vehículo
// ===============================================
  ...
  try {
    ...
    const [result] = await pool.query(
      `INSERT INTO Vehiculo (placa, modelo, tipo, kilometrajeActual, kilometrajeLimiteMantenimiento)
       VALUES (?, ?, ?, ?, ?)`,
      [placa, modelo, tipo, kilometrajeActual, kilometrajeLimiteMantenimiento]
    );
  ...
  try {
    ...
    if (error.code === 'ER_ROW_IS_REFERENCED_2') {
      return res.status(400).json({
        error: 'No se puede eliminar: este vehículo tiene viajes registrados'
      });
    }
